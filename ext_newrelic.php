...
function newrelic_fread(resource $handle, int $length) {
    if (stream_get_meta_data($handle)['wrapper_type'] != 'plainfile' && stream_get_meta_data($handle)['wrapper_type'] != 'PHP') {
...
function newrelic_socket_read(resource $socket, int $length, int $type = PHP_BINARY_READ) {
    if (stream_get_meta_data($socket)['wrapper_type'] != 'plainfile' && stream_get_meta_data($socket)['wrapper_type'] != 'PHP') {
